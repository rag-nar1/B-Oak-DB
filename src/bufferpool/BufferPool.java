...
public class BufferPool implements Closeable {
  ...
  /**
   * allocate a new page in the passed file and return the pageid if the file has deallocated pages
   * it returns one of them to prrevent Fragmentation
   *
   * @param fileName
   ...
   */
  private boolean diskOp(Frame frame, boolean isWrite)
      throws InterruptedException, NullPointerException, ExecutionException {
    DiskRequest request =
        new DiskRequest(frame.getFileName(), frame.getPageId(), frame.getData(), isWrite);
  ...
  private int getFrame()
      throws Exception, InterruptedException, NullPointerException, ExecutionException {
    int frameId;
   ...
   * @throws ExecutionException
   */
  private Guard getGuard(String fileName, long pageId, boolean isWrite)
      throws Exception, InterruptedException, NullPointerException, ExecutionException {
    bpmLatch.lock();
    if (pageId < 0) {
      bpmLatch.unlock();
   ...
   * @throws ExecutionException
   */
  public ReadGuard getReadGuard(String fileName, long pageId)
      throws Exception, InterruptedException, NullPointerException, ExecutionException {
   ...
   * @throws ExecutionException
   */
  public WriteGuard getWriteGuard(String fileName, long pageId)
      throws Exception,
