...
public class BasicDiskManager implements DiskManager {
  ...
  /** defualt constructre todo: make new constructors based on a config */
  public BasicDiskManager() throws NullPointerException {
    ...
    mainThread =
        new Thread(
            () -> { // start the worker that would fetch the requests and start a thread for each
              try {
                run();
  ...
  /**
   * closes all files and invalidate the requestQueue and the files map used as a destractor
   *
   * @throws NullPointerException if the object used after calling the close method for the first
   *     time
   */
